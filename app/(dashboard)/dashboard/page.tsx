    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="space-y-1">
          <h1 className="text-3xl font-black text-foreground tracking-tighter">Panel de Operaciones</h1>
          <div className="flex items-center gap-2 text-muted-foreground font-bold text-sm">
            <span>SendDirect x {user?.name || 'Equipo Directo'}</span>
            <span className="w-1.5 h-1.5 rounded-full bg-slate-200 dark:bg-slate-700" />
            <span className="flex items-center gap-1 cursor-pointer hover:text-foreground transition-colors uppercase tracking-widest text-[10px]">ID S-DIRECT-08PX <ArrowUpRight className="h-3 w-3" /></span>
          </div>
        </div>
        {user?.role === 'admin' ? (
          <Button 
        ...
        <div className="lg:col-span-2 space-y-6 bg-card p-8 rounded-[2.5rem] border border-border/50 shadow-sm text-card-foreground">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="p-6 rounded-4xl bg-blue-500/5 dark:bg-blue-900/10 border border-blue-500/10 dark:border-blue-900/20 flex flex-col gap-6">
              ...
              <div className="space-y-1">
                <p className="text-2xl font-black">{formatCurrency(stats?.totalBalance || 0)}</p>
            ...
            <div className="p-6 rounded-4xl bg-purple-500/5 dark:bg-purple-900/10 border border-purple-500/10 dark:border-purple-900/20 flex flex-col gap-6">
              ...
              <div className="space-y-1">
                <p className="text-2xl font-black">{formatCurrency(stats?.totalSent || 0)}</p>
            ...
            {recentTransfers.slice(0, 3).map((item, idx) => (
              <div key={item.id} className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className="text-sm font-black">{formatCurrency(item.amount, item.currency)}</p>
